...
{
	return g.m_bloom.contains(u.str()) > g.m_depthThresh;
}

/** Return whether this vertex exists in the subgraph. */
template <typename Graph>
static inline bool
vertex_exists(const btllib::NtHash& nthash, const Graph& g)
{
	return g.m_bloom.contains(nthash.hashes()) > g.m_depthThresh;
}
